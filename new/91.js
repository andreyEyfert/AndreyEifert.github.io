...
WndNds = function ( v ) {
    ...
    this.bind( 'nds price', function ( nds, price ) {
        ...
        v.items = Object.keys( nds ).map( k => (
            ...
            nds[ k ].list && Object.keys( nds[ k ].list ).forEach ( id => 
                t += price[ id ].price * nds[ k ].list[ id ] ),  
                v.month == nds[ k ].month && ( v.total += ( t * nds[ k ].size ) ),     
            {
                key: k,
                date: nds[ k ].date,
                total: ( t * nds[ k ].size ).toFixed( 2 ),
                month: nds[ k ].month,
                size: nds[ k ].size
            }) )
